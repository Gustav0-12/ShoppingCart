## SHOPPINGCART
Este é um projeto de um sistema básico de carrinho de compras, desenvolvido em Java utilizando o framework Spring e banco de dados MySQL. Ele permite o gerenciamento de produtos e clientes, facilitando o processo de compra.
...
```
### Carrinho de compras
``` Adicionar produto no carrinho: POST api/shoppingcart/{userId}/add/{productId} ```

``` Remover produto do carrinho: DELETE api/shoppingcart/{userId}/remove/{productId} ```

``` Encontrar carrinho do usuário por id: GET api/shoppingcart/user/{userId} ```
```Json
{
    "id": 1,
