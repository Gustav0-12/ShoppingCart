...
public class ProductService {
    ...
    public List<ProductResponseDTO> findAll() {
        ...
        return products.stream().map(product -> new ProductResponseDTO(product.getId(), product.getName(), product.getDescription(), product.getPrice(),
                product.getCategory().stream().map(
    ...
    public ProductResponseDTO create(ProductRequestDTO product) {
        ...
        if (products.isPresent()) {
            throw new UniqueViolationException("Esse produto já existe");
        }
        ...
        return new ProductResponseDTO(newProduct.getId(), newProduct.getName(), newProduct.getDescription(), newProduct.getPrice(), newProduct.getCategory().stream().map(category -> new CategoryDTO(category.getId(), category.getName())).collect(Collectors.toSet()));
    }
}
